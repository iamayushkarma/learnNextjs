...
export default function SignupPage() {
  ...
  const onSignup = async () => {
    ...
    try {
      ...
      console.log("Signup done", responce.data);
      setTimeout(() => {
        router.push("/login");
      }, 500);
    } catch (error: any) {
  ...
  return (
    <div className="flex flex-col items-center w-96 m-auto min-h-screen justify-center py-2">
      ...
      <div className="min-h-4/5 px-8 py-16 bg-[#09090b] border border-white/10 hover:border-white/20 focus-within:border-white/30 transition-colors duration-200 rounded-lg p-3">
        ...
        <div className="flex h-[40px] text-[15px] text-white/60 w-[280px] items-center bg-[#09090b] border border-white/10 rounded-lg focus-within:ring-2 focus-within:ring-gray-700 focus-within:ring-offset-2 focus-within:ring-offset-[#09090b] transition-all duration-150 ease-in-out">
          <div className="ml-[7.5px]">
            <User height="1.4em" width="1.4em" />
          </div>
          <input
            ...
            className="bg-transparent text-[#f4f4f5] px-2 py-1 rounded-l-lg focus:outline-none w-full"
          />
        </div>
        {errors.username && (
        ...
        <div className="relative top-6">
          <button
            ...
            onClick={onSignup}
          >
            {buttonDisabled ? "Waiting for input" : "Sign up"}
          </button>
